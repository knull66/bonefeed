import Foundation

enum AppLanguage: String, CaseIterable, Identifiable, Codable, Sendable {
    case english = "en"
    case spanish = "es"

    var id: String { rawValue }

    /// Always bilingual labels in the picker itself.
    var menuLabel: String {
        switch self {
        case .english: "English"
        case .spanish: "Español"
        }
    }
}

/// Lightweight in-app strings. Default language is English.
enum L10n {
    /// Mirrored from AppStore so services can localize without a store ref.
    nonisolated(unsafe) static var language: AppLanguage = .english

    static func t(_ key: String, lang: AppLanguage? = nil) -> String {
        let l = lang ?? language
        if let s = table[l]?[key] { return s }
        return table[.english]?[key] ?? key
    }

    private static let table: [AppLanguage: [String: String]] = [
        .english: en,
        .spanish: es
    ]

    // MARK: - English (default)

    private static let en: [String: String] = [
        // Brand / chrome
        "brand.tagline": "notch radar for crypto",
        "brand.settingsTitle": "Bonefeed Settings",
        "brand.quit": "Quit Bonefeed",

        // Settings sidebar
        "settings.section.main": "Main",
        "settings.section.appearance": "Appearance",
        "settings.pane.general": "General",
        "settings.pane.pro": "Pro / VIP",
        "settings.pane.display": "Display",
        "settings.pane.radar": "Radar",
        "settings.pane.binance": "Binance",
        "settings.pane.wallets": "Wallets",
        "settings.pane.alerts": "Alerts",
        "settings.pane.language": "Language",
        "settings.pane.guide": "Guide",
        "settings.pane.privacy": "Privacy",
        "settings.pane.about": "About",

        // Pro / Free
        "pro.status.free": "Bonefeed Free",
        "pro.status.pro": "Bonefeed Pro",
        "pro.status.freeDetail": "Ticker + 1 wallet. Upgrade for Binance Spot/Earn.",
        "pro.status.proDetail": "Full radar unlocked on this Mac.",
        "pro.freeTitle": "Free includes",
        "pro.free.1": "Live notch ticker (watchlist prices)",
        "pro.free.2": "Up to 3 watchlist coins",
        "pro.free.3": "1 on-chain wallet (BTC / ETH-MetaMask / SOL)",
        "pro.free.4": "Themes: Cyber Dim + Mac Dark",
        "pro.proTitle": "Pro unlocks",
        "pro.pro.1": "Binance API — Spot, Funding, deposits, live stream",
        "pro.pro.2": "Earn positions, rewards, unlock alerts",
        "pro.pro.3": "Unlimited wallets & watchlist",
        "pro.pro.4": "All themes (Binance, Mac Light, Neon…)",
        "pro.pro.5": "Portfolio PnL history in the notch",
        "pro.pro.6": "Binance P2P order alerts (new / paid / done / cancel / appeal)",
        "pro.buy": "Buy Bonefeed Pro",
        "pro.restore": "Restore Purchases",
        "pro.purchasing": "Opening App Store…",
        "pro.restoring": "Restoring…",
        "pro.unlocked": "Pro unlocked. Thanks!",
        "pro.restored": "Purchases restored — Pro is active.",
        "pro.restoredVIP": "Purchases restored — VIP Signals is active.",
        "pro.restoredNone": "Restore finished — no Pro/VIP on this Apple ID.",
        "pro.purchaseFailed": "Purchase didn’t complete.",
        "pro.restoreFailed": "Couldn’t restore purchases.",
        "pro.thanks": "Pro is active on this Apple ID.",
        "pro.storeHint": "One-time purchase via App Store. Managed by your Apple ID — Restore on a new Mac.",
        "pro.unlockDebug": "Pre-release: Unlock Pro",
        "pro.lockTest": "Pre-release: Back to Free",
        "pro.debugNote": "Local unlock until App Store IAP goes live. Disable allowLocalUnlock before release.",
        "pro.unlockedDebug": "Pro unlocked (pre-release).",
        "pro.locked": "Back to Free plan.",
        "pro.gate.title": "Pro feature",
        "pro.gate.binance": "Binance Spot / Earn / P2P needs Bonefeed Pro.",
        "pro.gate.wallets": "Free plan: 1 wallet. Unlock Pro for more.",
        "pro.gate.walletsLimit": "Free: {n} on-chain wallet. Pro = unlimited (incl. MetaMask).",
        "pro.gate.watchlist": "Free plan: 3 coins on the radar. Pro unlocks more.",
        "pro.gate.theme": "That theme is Pro. Unlock in Settings → Pro.",
        "pro.seePlan": "See Pro plan",
        "pro.panelLock": "SPOT & EARN need Pro — buy in Settings → Pro",

        // VIP Signals
        "vip.status": "Bonefeed VIP",
        "vip.statusDetail": "Tight signal desk active — still watch-only.",
        "vip.title": "VIP Signals",
        "vip.blurb": "Faster market pings (±2.5% pump/dump, fee desk). Never trades for you.",
        "vip.1": "Tight desk: ±2.5% pump / dump on your watchlist",
        "vip.2": "Earlier BTC fee warnings (25 sat/vB)",
        "vip.3": "Shorter alert cooldown (15 min)",
        "vip.4": "Includes everything in Pro",
        "vip.5": "Watch-only — alerts you act on",
        "vip.buy": "Buy Bonefeed VIP",
        "vip.purchasing": "Opening App Store…",
        "vip.unlocked": "VIP Signals unlocked. Desk is live.",
        "vip.purchaseFailed": "VIP purchase didn’t complete.",
        "vip.unlockDebug": "Pre-release: Unlock VIP",
        "vip.unlockedDebug": "VIP unlocked (pre-release).",
        "vip.deskToggle": "VIP tight desk",
        "vip.deskHelp": "When on: ±2.5% moves + fee desk. When off: use Free/Pro thresholds below.",
        "vip.gate": "VIP Signals unlocks the tight desk. Still watch-only — never places trades.",
        "vip.seePlan": "See VIP plan",

        "signals.blurb": "Not a crystal ball — an early-warning desk. When a coin crosses your threshold, you get the ping. You decide.",
        "signals.blurbShort": "Early-warning board. Cross the ring → you get the ping.",
        "signals.heroVip": "VIP TIGHT DESK",
        "signals.heroStd": "STANDARD DESK",
        "signals.nearHelp": "Heat → trigger",
        "signals.boardTitle": "HEAT BOARD",
        "signals.liveTitle": "LIVE PINGS",
        "signals.statHot": "HOTTEST",
        "signals.statHeat": "DESK HEAT",
        "signals.statHeatDetail": "avg to trigger",
        "signals.statFire": "FIRED",
        "signals.statFireDetail": "over threshold",
        "signals.actionTitle": "%@ · %@ — threshold hit",
        "signals.actionBody": "Open Binance if you want to act. Bonefeed will not trade.",
        "signals.calm": "Desk calm — nothing at trigger yet.",
        "signals.disclaimer": "Watch-only. Not financial advice. Does not predict or place trades.",
        "signals.openDesk": "Open SIGNAL desk →",
        "signals.openDeskLocked": "Open SIGNAL desk (VIP tight) →",
        "signals.vipUpsell": "STD floor ±5%. VIP unlocks ±2.5%, heat board, ping history.",
        "signals.lockedTitle": "VIP DESK LOCKED",
        "signals.lockedBody": "Heat board, FIRE strip, ping history and Binance jump are VIP-only. Free keeps overview signals at ±5%.",
        "signals.lockedCta": "Unlock the tight desk",
        "signals.quietOn": "Quiet hours — market pings muted (P2P/deposits still fire).",
        "signals.historyTitle": "PING HISTORY",
        "signals.historyEmpty": "No VIP pings yet — history fills when the desk fires.",
        "signals.historyClear": "Clear",
        "signals.openBinance": "Open Binance",
        "signals.openP2P": "Open P2P",
        "signals.openEarn": "Open Earn",
        "signals.markSeen": "Mark seen",
        "signals.fire": "FIRE",

        "tab.overview": "OVERVIEW",
        "tab.signals": "SIGNALS",
        "tab.p2p": "P2P",
        "tab.lab": "BOT",
        "tab.bot": "BOT",
        "panel.live": "▶ LIVE",
        "panel.sync": "… SYNC",
        "panel.paused": "PAUSED",
        "panel.boot": "BOOT SEQUENCE",
        "alerts.vipFloorHint": "Free/Pro floor is ±5% and fee ≥40. VIP tightens to ±2.5% / 25 sat.",

        // Superadmin broker bot (suggestions only)
        "lab.toggle": "Superadmin BOT (broker suggestions)",
        "lab.toggleHelp": "Shows RADAR → BOT: live broker-style suggestions. Never executes trades. Admin only.",
        "lab.open": "Open BOT desk",
        "bot.heroTitle": "BROKER BOT // ADMIN",
        "bot.heroBody": "Reads your desk and suggests what a broker might say next. Confidence is heuristic — not a signal service.",
        "bot.heroRule": "SUGGEST ONLY · never places orders · you click Binance if you act",
        "bot.mandate.title": "Mandate",
        "bot.mandate.body": "This bot never trades. Buttons only open Binance or dismiss the card. You stay in control.",
        "bot.stance.hold": "HOLD / WAIT",
        "bot.stance.watchLong": "WATCH LONG",
        "bot.stance.watchShort": "WATCH SHORT",
        "bot.stance.standAside": "STAND ASIDE",
        "bot.stance.ops": "OPS",
        "bot.stance.fee": "FEE RISK",
        "bot.action.signals": "Signals",
        "bot.action.dismiss": "Dismiss",
        "bot.p2pLive.title": "P2P order is live — ops first",
        "bot.p2pLive.body": "Ignore market noise until the P2P timer is safe. Finish the order flow before any discretionary trade idea.",
        "bot.fee.title": "BTC fees are expensive",
        "bot.fee.body": "Avoid unnecessary on-chain moves. Prefer staying in Earn/Spot unless the transfer is mandatory.",
        "bot.broadDump.title": "Broad risk-off — stand aside",
        "bot.broadDump.body": "%d/%d coins dumped · BTC %@%%. Suggestion: do not chase knives. One market event, not five trades.",
        "bot.broadDumpShort.title": "Optional: watch short continuation",
        "bot.broadDumpShort.body": "Only if you already had a short plan. Otherwise skip. Bot will not open a position.",
        "bot.idioDump.title": "Single-name dump — low conviction",
        "bot.idioDump.body": "%@ %@%% without a volume spike. Suggestion: wait for reason (news/unlock) before any short idea.",
        "bot.idioDumpVol.title": "Single-name dump + volume",
        "bot.idioDumpVol.body": "%@ %@%% with elevated 15m volume. Suggestion: watch for continuation short — confirm on Binance chart first.",
        "bot.clusterPump.title": "Clustered pumps — don't FOMO",
        "bot.clusterPump.body": "%d pumps live. Suggestion: stand aside unless you planned size before the green candles.",
        "bot.clusterPumpLong.title": "Optional: watch long pullback entry",
        "bot.clusterPumpLong.body": "If you chase, you are late. Prefer wait for a pullback. Bot does not buy.",
        "bot.singlePump.title": "Single pump — wait confirmation",
        "bot.singlePump.body": "%@ %@%%. Suggestion: hold/wait for a second confirmation (retest / BTC not dumping).",
        "bot.singlePumpVol.title": "Single pump + volume",
        "bot.singlePumpVol.body": "%@ %@%% with volume. Suggestion: watch long only if your plan already exists.",
        "bot.btcLeadDown.title": "BTC leading lower",
        "bot.btcLeadDown.body": "BTC %@%%. Alts usually follow. Suggestion: bias watch-short / reduce long aggression.",
        "bot.btcLeadUp.title": "BTC leading higher",
        "bot.btcLeadUp.body": "BTC %@%%. Suggestion: bias watch-long on strength — still not an order.",
        "bot.near.title": "Near trigger — pre-commit",
        "bot.near.body": "Desk heat ~%d%%. Decide now: ignore, or what you would do if it fires. Don't invent a plan after the ping.",
        "bot.calm.title": "Tape calm — no edge forced",
        "bot.calm.body": "Nothing at trigger. Suggestion: hold cash/plan, review watchlist, don't invent trades from boredom.",
        "bot.quiet.title": "Quiet hours — stand aside",
        "bot.quiet.body": "Your quiet hours are on. Suggestion: no discretionary ideas until the session opens.",
        "bot.noApi.title": "Binance API off",
        "bot.noApi.body": "Connect a read-only key for portfolio/P2P context. Bot suggestions are weaker without ops data.",
        "bot.vipNoise.title": "VIP desk is noisy",
        "bot.vipNoise.body": "Tight ±2.5% is firing often. Suggestion: treat as heat map, not as N separate trades.",
        "lab.heroTitle": "TRADING LAB // SUPERADMIN",
        "lab.heroBody": "Live coaching for the operator. Reads the desk and asks better questions — it never tells you to buy or sell.",
        "lab.heroRule": "Rule: observe → journal → decide later. Bonefeed stays watch-only.",
        "lab.card.frame.title": "How to use this desk",
        "lab.card.frame.body": "Each DUMP/PUMP is a classroom bell, not an order. Ask: correlated or idiosyncratic? Did I already have a plan?",
        "lab.card.regimeDump.title": "Broad risk-off (many dumps)",
        "lab.card.regimeDump.body": "%d / %d coins over dump threshold. Treat as ONE market event. Counting five separate emergencies is the beginner trap.",
        "lab.card.beginnerDump.title": "Beginner drill — dump flood",
        "lab.card.beginnerDump.body": "Practice: (1) freeze 60s (2) check if BTC also red (3) write one sentence: ‘I will do nothing / I had a plan for X’. Do not revenge-trade.",
        "lab.card.idioDump.title": "Single-name pressure",
        "lab.card.idioDump.body": "%@ at %@%% while the board is not fully red. Ask why THIS coin: unlock, news, leverage flush — not ‘the market died’.",
        "lab.card.regimePump.title": "Clustered pumps",
        "lab.card.regimePump.body": "%d pumps live. Drill: name your FOMO urge out loud, then check if you had size/plan before the green candles.",
        "lab.card.singlePump.title": "Single pump ping",
        "lab.card.singlePump.body": "One green alert ≠ thesis. Practice waiting for a second confirmation (retest, volume, BTC not dumping) before any idea of acting.",
        "lab.card.fees.title": "Fee desk is hot",
        "lab.card.fees.body": "High sats = on-chain moves are expensive. Learning: separate ‘I want to move coins’ from ‘price is moving’. Fees punish urgency.",
        "lab.card.btcLead.title": "BTC is leading",
        "lab.card.btcLead.body": "BTC 24h %@%%. Most alts are beta to BTC. If BTC dumps, alts dumping with it is normal — not five unique crises.",
        "lab.card.near.title": "Near trigger (heat)",
        "lab.card.near.body": "Desk heat ~%d%%. Skill: watch how you feel BEFORE fire. Pre-commit: what would I do if it crosses?",
        "lab.card.calm.title": "Calm tape — use it",
        "lab.card.calm.body": "No fires. Best time to study: journal last pings, tighten watchlist, rehearse your rules without adrenaline.",
        "lab.card.quiet.title": "Quiet hours on",
        "lab.card.quiet.body": "Market pings muted. Ops lesson: pros schedule attention. Sleep > staring at a red board at 3am.",
        "lab.card.vipNoise.title": "VIP sensitivity",
        "lab.card.vipNoise.body": "±2.5% fires often. Learning: sensitivity ≠ edge. If everything alerts, raise the bar or shorten the watchlist.",
        "lab.card.history.title": "Review your ping log",
        "lab.card.history.body": "%d stored pings. Drill: mark which ones you would have acted on vs ignored. Pattern > prediction.",
        "lab.card.skill.title": "Skill of the day",
        "lab.card.disclaimer.title": "Not financial advice",
        "lab.card.disclaimer.body": "LAB is a private training overlay for the admin. It does not recommend trades, size, or timing. You are responsible for every decision.",
        "lab.skill.r1": "Write a one-line trade plan BEFORE any alert. If no plan exists when it fires — default is observe only.",
        "lab.skill.r2": "Separate P2P/ops urgency from market noise. A dump alert is not a P2P timer.",
        "lab.skill.r3": "Practice position sizing on paper: ‘If I were wrong, what % hurts sleep?’ If unclear, size is too big.",
        "lab.skill.r4": "Learn fee friction: every on-chain move has a cost. Urgency is how beginners overpay.",
        "lab.skill.r5": "After a dump flood, wait one full candle close (your timeframe) before changing any thesis.",
        "lab.concept.radar": "Concept: radar ≠ signal to act",
        "lab.concept.correlation": "Concept: correlation / regime",
        "lab.concept.plan": "Concept: plan before trigger",
        "lab.concept.idiosyncratic": "Concept: idiosyncratic risk",
        "lab.concept.fomo": "Concept: FOMO control",
        "lab.concept.confirmation": "Concept: confirmation",
        "lab.concept.fees": "Concept: fee awareness",
        "lab.concept.btcBeta": "Concept: BTC beta",
        "lab.concept.threshold": "Concept: threshold discipline",
        "lab.concept.journal": "Concept: journal in calm",
        "lab.concept.session": "Concept: session management",
        "lab.concept.sensitivity": "Concept: alert sensitivity",
        "lab.concept.review": "Concept: post-trade review",
        "lab.concept.practice": "Concept: deliberate practice",

        // General
        "general.system": "System",
        "general.launchAtLogin": "Launch at Login",
        "general.openLoginItems": "Open Login Items…",
        "general.loginBlocked": "macOS blocked login launch. Approve Bonefeed in Login Items.",
        "general.runtime": "Runtime",
        "general.pause": "Pause live feed",
        "general.sound": "Sound effects",
        "general.pollHint": "Poll: ~10–15s. Watch-only — never trades.",

        // Display
        "display.theme": "Theme",
        "display.themeHint": "Theme applies to notch + panel. Binance & Mac themes included.",

        // Language
        "language.title": "App language",
        "language.hint": "UI copy for Settings, guide, alerts and status. Cyber labels (RADAR / SPOT) stay the same.",
        "language.defaultNote": "Default is English.",

        // Guide
        "guide.title": "Usage guide",
        "guide.blurb": "Bonefeed watches balances and alerts you. It does not buy, sell, or sign.",
        "guide.step1.title": "BINANCE",
        "guide.step1.body": "Settings → Binance → paste a read-only API key → Save → Test.",
        "guide.step2.title": "NOTCH",
        "guide.step2.body": "Click the island for PORT / EARN / SIG. It sits in the menu-bar notch.",
        "guide.step3.title": "LOG",
        "guide.step3.body": "When it pulses, open LOG — deposits and signals land there.",
        "guide.step4.title": "SIGNALS",
        "guide.step4.body": "RADAR → SIGNALS is the early-warning desk. VIP unlocks the tight heat board.",
        "guide.openPanel": "Show guide on panel",
        "guide.footnote": "Watch-only · no trade · no sign",

        // Privacy / terms
        "privacy.termsTitle": "Terms of use",
        "privacy.privacyTitle": "Privacy",
        "privacy.termsBody": """
Bonefeed is a watch-only informational tool for personal use.

• It does not execute trades, transfers, approvals, or signatures.
• You are solely responsible for API keys, wallets you add, and decisions you make from the data shown.
• Market prices and balances come from third parties (e.g. Binance, mempool.space, public RPCs) and may be delayed or wrong.
• Nothing in the app is financial, investment, or tax advice.
• The software is provided “as is”, without warranty. Use at your own risk.
• Do not use a Binance API key with withdraw or trade permissions.
""",
        "privacy.privacyBody": """
What we store — only on this Mac:

• Binance API key/secret encrypted locally (AES-GCM; wrapping key in Keychain).
• Watchlist, wallets, alert thresholds, theme, language, and panel position in UserDefaults / app support.
• Optional portfolio snapshots for 24h PnL.

What we do not do:

• No Bonefeed cloud account. No analytics SDK. No sale of personal data.
• Credentials are not uploaded to our servers (there are none for this app).
• Network calls go to exchanges / public chain APIs you configure by using the feature (Binance, mempool, RPC).

You can clear Binance credentials anytime in Settings → Binance → Clear.
""",
        "privacy.ack": "By using Bonefeed you agree to these terms and the privacy notes above.",

        // Radar settings
        "radar.watchlist": "Market watchlist",
        "radar.watchlistHint": "24h prices & signals on RADAR (Binance public ticker).",
        "radar.custom": "Custom (e.g. ARB)",
        "radar.add": "Add",
        "radar.active": "Active:",
        "radar.reset": "Reset to BTC · ETH · SOL",
        "radar.stablesNote": "Stables (USDT…) can't be watched as tickers.",
        "radar.onchain": "On-chain",
        "radar.onchainHint": "Watch-only BTC / ETH (+Aave HF) / SOL. Binance deposits cover any coin on credit.",

        // Binance
        "binance.apiTitle": "API (read-only)",
        "binance.apiHint": "Binance read-only API key. Bonefeed can view balances — never withdraw or trade.",
        "binance.save": "Save",
        "binance.test": "Test",
        "binance.clear": "Clear",
        "binance.encrypted": "Encrypted on this Mac. Read-only key only — never share your secret.",
        "binance.streamLive": "User data stream live — faster deposit refresh.",

        // Wallets
        "wallets.watch": "Watch address (on-chain)",
        "wallets.hint": "Watch-only addresses. Balance + deposit alerts. No seed / no spend. MetaMask = paste your ETH account address.",
        "wallets.metamaskTip": "MetaMask → Account details → Copy address. Paste the 0x… here. Watch-only — Bonefeed never connects or signs.",
        "wallets.metamaskLabel": "Use label “MetaMask”",
        "wallets.chain": "Chain",
        "wallets.label": "Label",
        "wallets.add": "Add wallet",
        "wallets.empty": "No wallets watched.",
        "wallets.remove": "Remove",
        "wallets.invalidBTC": "Use a BTC address (bc1… / 1… / 3…)",
        "wallets.invalidETH": "Use an ETH / MetaMask address 0x… (40 hex)",
        "wallets.invalidSOL": "Use a SOL base58 address (32–44 chars)",
        "wallets.invalidTitle": "Invalid address",

        // Alerts
        "alerts.notifications": "macOS Notifications",
        "alerts.enable": "Enable notifications",
        "alerts.request": "Request permission",
        "alerts.openSystem": "Open System Settings…",
        "alerts.test": "Test notify",
        "alerts.signals": "Signals",
        "alerts.pumpDump": "24h pump/dump (watchlist)",
        "alerts.dump": "Dump ≤",
        "alerts.pump": "Pump ≥",
        "alerts.signalAssets": "Signal assets (empty = all watchlist):",
        "alerts.filterOff": "Filter off · all radar assets.",
        "alerts.fee": "High BTC fee alert",
        "alerts.feeAt": "Fee ≥",
        "alerts.delivery": "Delivery",
        "alerts.cooldown": "Cooldown",
        "alerts.quiet": "Quiet hours",
        "alerts.from": "From",
        "alerts.to": "to",
        "alerts.quietHint": "Mute pump/dump/fee/earn/health. Deposits + P2P still notify.",
        "alerts.antiFlap": "Anti-flap (hysteresis)",
        "alerts.antiFlapHint": "Fire once at threshold; re-arm only after price recovers (~72% of the band).",
        "alerts.volumeFilter": "Volume spike filter (3×)",
        "alerts.volumeFilterHint": "Only notify pump/dump when 15m volume ≥ 3× the prior average.",
        "alerts.focus": "Respect macOS Focus / DND",
        "alerts.focusHint": "Mute market noise while Focus is on. Deposits + P2P still notify.",
        "alerts.p2pRate": "P2P rate watch",
        "alerts.p2pRateHint": "Requires a specific fiat (not Auto). Alerts when best ad crosses your line.",
        "alerts.p2pRateBuyMax": "Buy ≥ (fiat/unit)",
        "alerts.p2pRateSellMin": "Sell ≤ (fiat/unit)",
        "alerts.aave": "Aave (ETH)",
        "alerts.aaveHint": "Aave V3 health on watched ETH wallets. Ignore if you don't use Aave.",
        "alerts.health": "Health factor alerts",
        "alerts.healthWarn": "Warn if HF <",
        "alerts.p2p": "Binance P2P",
        "alerts.p2pHint": "Pro · watch-only. Notifies on order status changes. Needs Binance API Reading.",
        "alerts.p2pEnable": "P2P order alerts",
        "alerts.p2pFiat": "Fiat filter",
        "alerts.p2pFiatAuto": "Auto (all)",
        "msg.p2pNew": "P2P · NEW",
        "msg.p2pPaid": "P2P · PAID",
        "msg.p2pRelease": "P2P · RELEASING",
        "msg.p2pDone": "P2P · DONE",
        "msg.p2pCancel": "P2P · CANCEL",
        "msg.p2pAppeal": "P2P · APPEAL",
        "msg.p2pStatus": "P2P · UPDATE",
        "msg.p2pRateBuy": "P2P · BUY RATE",
        "msg.p2pRateBuyDetail": "%@/%@ best buy %.4f ≥ %.4f",
        "msg.p2pRateSell": "P2P · SELL RATE",
        "msg.p2pRateSellDetail": "%@/%@ best sell %.4f ≤ %.4f",
        "menu.section.panel": "Panel",
        "menu.section.radar": "Radar",
        "menu.section.quick": "Quick links",
        "menu.showPanel": "Show Panel",
        "menu.hidePanel": "Hide Panel",
        "menu.openOverview": "Open Overview",
        "menu.openP2P": "Open P2P status",
        "menu.openSignals": "Open SIGNAL desk",
        "menu.openLab": "Open BOT (superadmin)",
        "menu.openBot": "Open BOT (superadmin)",
        "menu.settings": "Settings…",

        // About
        "about.mode": "Mode",
        "about.modeValue": "Market Radar + Binance + Earn",
        "about.chains": "Chains",
        "about.chainsValue": "BTC · ETH (+Aave HF) · SOL",
        "about.data": "Data",
        "about.dataValue": "Binance · mempool · RPC · user stream",
        "about.secrets": "Secrets",
        "about.secretsValue": "AES-GCM + Keychain wrap",
        "about.footer": "Settings here; island panel stays operational.",
        "about.showGuide": "Open usage guide…",
        "about.madeBy": "Made by",
        "about.studio": "Vibes District",
        "about.site": "Website",

        // Panel / notch chrome (keep cyber EN short where possible)
        "panel.refreshHint": "Force refresh Spot / Earn / markets",
        "panel.pnlBag": "bag",
        "panel.pnlMkt": "mkt",
        "panel.tapLive": "tap LIVE to refresh",
        "panel.apiOff": "API off · ⚙ Binance",
        "panel.marketsEmpty": "EMPTY · ⚙ Radar watchlist",
        "panel.logEmpty": "LOG EMPTY",
        "panel.logHint": "Deposits, P2P & earn events appear here.",
        "panel.p2pOff": "P2P alerts off · ⚙ Alerts",
        "panel.p2pEmpty": "NO P2P ORDERS",
        "panel.p2pHint": "Watch-only. Alerts on NEW / PAID / DONE / CANCEL / APPEAL. Fiat filter in Settings → Alerts.",
        "panel.p2pDemoHint": "Test the live order card without a real transfer. Real C2C orders still appear when Binance is connected.",
        "panel.simulateP2P": "> SIMULATE_P2P",
        "panel.advanceP2P": "> ADVANCE_PHASE",
        "panel.clearP2PDemo": "> CLEAR_DEMO",
        "p2p.card.waitBuyerPay": "Pending buyer's payment…",
        "p2p.card.paySeller": "Please pay the seller…",
        "p2p.card.releaseCrypto": "Payment received — release crypto…",
        "p2p.card.waitRelease": "Waiting for seller to release…",
        "p2p.card.releasing": "Releasing crypto…",
        "p2p.card.done": "Trade completed",
        "p2p.card.cancelled": "Order cancelled",
        "p2p.card.appeal": "Order in appeal",
        "p2p.card.update": "Order updated",
        "panel.simulate": "> SIMULATE_DEPOSIT",
        "panel.clearLog": "> CLEAR_LOG",
        "panel.noEarn": "NO EARN POSITIONS",
        "panel.noApi": "NO API  →  ⚙ SETTINGS",
        "panel.spotEmpty": "EMPTY  (funds may be in EARN)",
        "panel.pin": "Pin — keep panel open",
        "panel.unpin": "Unpin",
        "notch.peekHint": "Click to open",
        "notch.p2pDockHint": "Open P2P order status",
        "notch.alertHint": "Click to open LOG",
        "notch.alertSignalsHint": "Click to open SIGNAL desk",
        "notch.closePanel": "Close panel",
        "notch.openPanel": "Open Bonefeed",

        // Onboarding overlay
        "onboard.title": "BOOT // FIRST RUN",
        "onboard.settings": "> OPEN_SETTINGS",
        "onboard.toSignals": "> OPEN_SIGNAL_DESK",
        "onboard.ack": "> ACK",

        // Runtime / store messages
        "msg.bootDetail": "Booting radar…",
        "msg.pasteKey": "Paste read-only API key and save.",
        "msg.keyRequired": "API key and secret required.",
        "msg.savedEncrypted": "Saved encrypted (no Keychain prompts).",
        "msg.saveFailed": "In-memory this session (save failed):",
        "msg.cleared": "Credentials cleared.",
        "msg.saveFirst": "Save keys first.",
        "msg.apiMemory": "API in memory.",
        "msg.apiReady": "API ready.",
        "msg.apiMigrated": "API migrated to encrypted store.",
        "msg.noApi": "Set API in Settings",
        "msg.noApiDetail": "No Binance API — on-chain only. Connect keys for Spot balance.",
        "msg.testNotifyBody": "If you see this, macOS notifications work.",
        "msg.simDeposit": "Simulated deposit",
        "msg.simDetail": "Sim — this is what an incoming deposit looks like.",
        "msg.earnSubscribe": "Earn subscribe",
        "msg.earnRedeem": "Earn redeem",
        "msg.unlockSoon": "Unlock soon",
        "msg.depositSimTitle": "Simulated deposit",

        // Deposits / signals
        "deposit.binanceOk": "Credited on Binance",
        "deposit.binancePending": "Binance deposit",
        "deposit.onchainOk": "On-chain confirmed",
        "deposit.onchainMem": "On-chain mempool",
        "deposit.sim": "Simulated deposit",
        "signal.feesHigh": "High fees",
        "signal.btcFees": "BTC fees high",
        "signal.dump": "dump 24h",
        "signal.pump": "pump 24h",
        "signal.signalDump": "Signal: %@ dump",
        "signal.signalPump": "Signal: %@ pump",
        "signal.regimeDump": "Market-wide dump",
        "signal.regimeDumpDetail": "%d / %d coins over dump threshold — one event, not N emergencies.",
        "signal.calm": "Market calm",
        "signal.healthLow": "Health low",
        "signal.aaveRisk": "Aave health risk",
        "signal.radarOffline": "Radar offline",
        "status.healthLow": "Aave health factor low — check collateral/debt.",
        "status.pendingBtc": "Unconfirmed BTC in mempool — Binance credits on confirm.",
        "status.zeroOnchain": "Radar live. On-chain balance 0 — alert on first deposit.",
        "status.listening": "Radar live. Listening for deposits…",
        "status.earnSettling": "Account still $%.2f w/ 0 positions (redeem settling?)",

        // Notifier / login
        "notify.readyBody": "Radar ready for deposits and Earn alerts.",
        "notify.notDetermined": "No permission yet — tap Enable notifications",
        "notify.denied": "Denied — enable in System Settings → Notifications",
        "notify.authorized": "Enabled (check System Settings if missing)",
        "notify.provisional": "Provisional — find Bonefeed in Notifications",
        "notify.ephemeral": "Ephemeral",
        "login.enabled": "On — will open at login",
        "login.disabled": "Off",
        "login.pending": "Pending: approve Bonefeed in Login Items",
        "login.notFound": "Use the app from ~/Applications and re-enable",

        // API errors
        "err.missingCreds": "Missing API key / secret",
        "err.badURL": "Invalid URL",
        "err.decoding": "Unreadable Binance response",
    ]

    // MARK: - Spanish

    private static let es: [String: String] = [
        "brand.tagline": "radar cripto en el notch",
        "brand.settingsTitle": "Ajustes de Bonefeed",
        "brand.quit": "Salir de Bonefeed",

        "settings.section.main": "Principal",
        "settings.section.appearance": "Apariencia",
        "settings.pane.general": "General",
        "settings.pane.pro": "Pro / VIP",
        "settings.pane.display": "Apariencia",
        "settings.pane.radar": "Radar",
        "settings.pane.binance": "Binance",
        "settings.pane.wallets": "Wallets",
        "settings.pane.alerts": "Avisos",
        "settings.pane.language": "Idioma",
        "settings.pane.guide": "Guía",
        "settings.pane.privacy": "Privacidad",
        "settings.pane.about": "Acerca de",

        "pro.status.free": "Bonefeed Free",
        "pro.status.pro": "Bonefeed Pro",
        "pro.status.freeDetail": "Ticker + 1 wallet. Pro desbloquea Binance Spot/Earn.",
        "pro.status.proDetail": "Radar completo desbloqueado en este Mac.",
        "pro.freeTitle": "Free incluye",
        "pro.free.1": "Ticker live en el notch (precios watchlist)",
        "pro.free.2": "Hasta 3 monedas en el radar",
        "pro.free.3": "1 wallet on-chain (BTC / ETH-MetaMask / SOL)",
        "pro.free.4": "Temas: Cyber Dim + Mac Dark",
        "pro.proTitle": "Pro desbloquea",
        "pro.pro.1": "API Binance — Spot, Funding, depósitos, stream",
        "pro.pro.2": "Earn, rewards y avisos de unlock",
        "pro.pro.3": "Wallets y watchlist ilimitados",
        "pro.pro.4": "Todos los temas (Binance, Mac Light, Neon…)",
        "pro.pro.5": "Historial PnL de portfolio en el notch",
        "pro.pro.6": "Alertas P2P Binance (nueva / pagada / done / cancel / appeal)",
        "pro.buy": "Comprar Bonefeed Pro",
        "pro.restore": "Restaurar compras",
        "pro.purchasing": "Abriendo App Store…",
        "pro.restoring": "Restaurando…",
        "pro.unlocked": "Pro desbloqueado. ¡Gracias!",
        "pro.restored": "Compras restauradas — Pro activo.",
        "pro.restoredVIP": "Compras restauradas — VIP Signals activo.",
        "pro.restoredNone": "Restore listo — no hay Pro/VIP en este Apple ID.",
        "pro.purchaseFailed": "La compra no se completó.",
        "pro.restoreFailed": "No se pudieron restaurar las compras.",
        "pro.thanks": "Pro activo en este Apple ID.",
        "pro.storeHint": "Pago único vía App Store. Lo gestiona tu Apple ID — Restaurar en otro Mac.",
        "pro.unlockDebug": "Pre-release: Desbloquear Pro",
        "pro.lockTest": "Pre-release: Volver a Free",
        "pro.debugNote": "Unlock local hasta que el IAP esté en App Store. Pon allowLocalUnlock = false antes de publicar.",
        "pro.unlockedDebug": "Pro desbloqueado (pre-release).",
        "pro.locked": "De vuelta al plan Free.",
        "pro.gate.title": "Función Pro",
        "pro.gate.binance": "Binance Spot / Earn / P2P requiere Bonefeed Pro.",
        "pro.gate.wallets": "Free: 1 wallet. Pro para más.",
        "pro.gate.walletsLimit": "Free: {n} wallet on-chain. Pro = ilimitadas (incl. MetaMask).",
        "pro.gate.watchlist": "Free: 3 monedas en el radar. Pro desbloquea más.",
        "pro.gate.theme": "Ese tema es Pro. Desbloquea en Ajustes → Pro.",
        "pro.seePlan": "Ver plan Pro",
        "pro.panelLock": "SPOT y EARN necesitan Pro — compra en Ajustes → Pro",

        "vip.status": "Bonefeed VIP",
        "vip.statusDetail": "Desk de señales activas — sigue siendo watch-only.",
        "vip.title": "VIP Signals",
        "vip.blurb": "Pings más rápidos (±2.5% pump/dump, fees). Nunca opera por ti.",
        "vip.1": "Desk tight: ±2.5% pump / dump en tu watchlist",
        "vip.2": "Avisos de fee BTC antes (25 sat/vB)",
        "vip.3": "Cooldown de alertas más corto (15 min)",
        "vip.4": "Incluye todo Pro",
        "vip.5": "Watch-only — tú decides",
        "vip.buy": "Comprar Bonefeed VIP",
        "vip.purchasing": "Abriendo App Store…",
        "vip.unlocked": "VIP Signals desbloqueado. Desk activo.",
        "vip.purchaseFailed": "La compra VIP no se completó.",
        "vip.unlockDebug": "Pre-release: Desbloquear VIP",
        "vip.unlockedDebug": "VIP desbloqueado (pre-release).",
        "vip.deskToggle": "Desk VIP tight",
        "vip.deskHelp": "On: ±2.5% y fee desk. Off: usa los umbrales Free/Pro de abajo.",
        "vip.gate": "VIP Signals desbloquea el desk tight. Sigue watch-only — nunca coloca trades.",
        "vip.seePlan": "Ver plan VIP",

        "signals.blurb": "No es bola de cristal — es un desk de alerta temprana. Cuando una moneda cruza tu umbral, te llega el ping. Tú decides.",
        "signals.blurbShort": "Tablero de alerta temprana. Cruza el anillo → llega el ping.",
        "signals.heroVip": "VIP TIGHT DESK",
        "signals.heroStd": "DESK STANDARD",
        "signals.nearHelp": "Heat → trigger",
        "signals.boardTitle": "HEAT BOARD",
        "signals.liveTitle": "PINGS LIVE",
        "signals.statHot": "HOTTEST",
        "signals.statHeat": "DESK HEAT",
        "signals.statHeatDetail": "prom. al trigger",
        "signals.statFire": "FIRED",
        "signals.statFireDetail": "sobre umbral",
        "signals.actionTitle": "%@ · %@ — umbral cruzado",
        "signals.actionBody": "Abre Binance si quieres actuar. Bonefeed no opera.",
        "signals.calm": "Desk calm — nada en trigger todavía.",
        "signals.disclaimer": "Watch-only. No es consejo financiero. No predice ni coloca trades.",
        "signals.openDesk": "Abrir desk SIGNAL →",
        "signals.openDeskLocked": "Abrir desk SIGNAL (VIP tight) →",
        "signals.vipUpsell": "Floor STD ±5%. VIP: ±2.5%, heat board e historial.",
        "signals.lockedTitle": "VIP DESK BLOQUEADO",
        "signals.lockedBody": "Heat board, strip FIRE, historial y salto a Binance son VIP. Free mantiene señales overview a ±5%.",
        "signals.lockedCta": "Desbloquear desk tight",
        "signals.quietOn": "Quiet hours — pings de mercado muteados (P2P/depósitos sí).",
        "signals.historyTitle": "HISTORIAL DE PINGS",
        "signals.historyEmpty": "Sin pings VIP aún — el historial se llena cuando dispara el desk.",
        "signals.historyClear": "Borrar",
        "signals.openBinance": "Abrir Binance",
        "signals.openP2P": "Abrir P2P",
        "signals.openEarn": "Abrir Earn",
        "signals.markSeen": "Marcar visto",
        "signals.fire": "FIRE",

        "tab.overview": "OVERVIEW",
        "tab.signals": "SIGNALS",
        "tab.p2p": "P2P",
        "tab.lab": "BOT",
        "tab.bot": "BOT",
        "panel.live": "▶ LIVE",
        "panel.sync": "… SYNC",
        "panel.paused": "PAUSED",
        "panel.boot": "BOOT SEQUENCE",
        "alerts.vipFloorHint": "Free/Pro: piso ±5% y fee ≥40. VIP aprieta a ±2.5% / 25 sat.",

        "lab.toggle": "Superadmin BOT (sugerencias broker)",
        "lab.toggleHelp": "Muestra RADAR → BOT: sugerencias tipo broker en vivo. Nunca ejecuta trades. Solo admin.",
        "lab.open": "Abrir desk BOT",
        "bot.heroTitle": "BROKER BOT // ADMIN",
        "bot.heroBody": "Lee tu desk y sugiere qué diría un broker. La confianza es heurística — no es un servicio de señales.",
        "bot.heroRule": "SOLO SUGIERE · nunca coloca órdenes · tú abres Binance si actúas",
        "bot.mandate.title": "Mandato",
        "bot.mandate.body": "Este bot nunca opera. Los botones solo abren Binance o descartan la card. El control es tuyo.",
        "bot.stance.hold": "HOLD / ESPERAR",
        "bot.stance.watchLong": "WATCH LONG",
        "bot.stance.watchShort": "WATCH SHORT",
        "bot.stance.standAside": "STAND ASIDE",
        "bot.stance.ops": "OPS",
        "bot.stance.fee": "FEE RISK",
        "bot.action.signals": "Signals",
        "bot.action.dismiss": "Descartar",
        "bot.p2pLive.title": "Orden P2P viva — ops primero",
        "bot.p2pLive.body": "Ignora ruido de mercado hasta que el timer P2P esté seguro. Termina el flujo antes de cualquier idea discrecional.",
        "bot.fee.title": "Fees BTC caras",
        "bot.fee.body": "Evita movimientos on-chain innecesarios. Prefiere Earn/Spot salvo que el transfer sea obligatorio.",
        "bot.broadDump.title": "Risk-off amplio — stand aside",
        "bot.broadDump.body": "%d/%d coins en dump · BTC %@%%. Sugerencia: no persigas cuchillos. Un evento de mercado, no cinco trades.",
        "bot.broadDumpShort.title": "Opcional: watch short continuación",
        "bot.broadDumpShort.body": "Solo si ya tenías plan de short. Si no, skip. El bot no abre posición.",
        "bot.idioDump.title": "Dump de un nombre — baja convicción",
        "bot.idioDump.body": "%@ %@%% sin spike de volumen. Sugerencia: espera motivo (news/unlock) antes de pensar short.",
        "bot.idioDumpVol.title": "Dump de un nombre + volumen",
        "bot.idioDumpVol.body": "%@ %@%% con volumen 15m alto. Sugerencia: watch short continuación — confirma en chart de Binance.",
        "bot.clusterPump.title": "Pumps en cluster — no FOMO",
        "bot.clusterPump.body": "%d pumps live. Sugerencia: stand aside salvo que hayas planeado size antes de las velas verdes.",
        "bot.clusterPumpLong.title": "Opcional: watch long en pullback",
        "bot.clusterPumpLong.body": "Si persigues, vas tarde. Mejor esperar pullback. El bot no compra.",
        "bot.singlePump.title": "Pump único — espera confirmación",
        "bot.singlePump.body": "%@ %@%%. Sugerencia: hold/espera segunda confirmación (retest / BTC no dump).",
        "bot.singlePumpVol.title": "Pump único + volumen",
        "bot.singlePumpVol.body": "%@ %@%% con volumen. Sugerencia: watch long solo si ya existía tu plan.",
        "bot.btcLeadDown.title": "BTC lidera a la baja",
        "bot.btcLeadDown.body": "BTC %@%%. Las alts suelen seguir. Sugerencia: sesgo watch-short / menos agresividad long.",
        "bot.btcLeadUp.title": "BTC lidera al alza",
        "bot.btcLeadUp.body": "BTC %@%%. Sugerencia: sesgo watch-long en fuerza — sigue sin ser una orden.",
        "bot.near.title": "Cerca del trigger — pre-compromiso",
        "bot.near.body": "Heat ~%d%%. Decide ya: ignorar, o qué harías si dispara. No inventes el plan después del ping.",
        "bot.calm.title": "Tape calm — no fuerces edge",
        "bot.calm.body": "Nada en trigger. Sugerencia: hold, revisa watchlist; no inventes trades por aburrimiento.",
        "bot.quiet.title": "Quiet hours — stand aside",
        "bot.quiet.body": "Quiet hours on. Sugerencia: cero ideas discrecionales hasta abrir sesión.",
        "bot.noApi.title": "API Binance off",
        "bot.noApi.body": "Conecta key read-only para contexto portfolio/P2P. Sin ops data el bot sugiere peor.",
        "bot.vipNoise.title": "Desk VIP ruidoso",
        "bot.vipNoise.body": "±2.5% dispara mucho. Sugerencia: trátalo como mapa de calor, no como N trades.",
        "lab.heroTitle": "TRADING LAB // SUPERADMIN",
        "lab.heroBody": "Coaching live para el operador.",
        "lab.heroRule": "Bonefeed sigue watch-only.",
        "lab.card.frame.title": "Cómo usar este desk",
        "lab.card.frame.body": "Cada DUMP/PUMP es un timbre de clase, no una orden. Pregunta: ¿correlacionado o idiosincrático? ¿Ya tenía plan?",
        "lab.card.regimeDump.title": "Risk-off amplio (muchos dumps)",
        "lab.card.regimeDump.body": "%d / %d coins sobre umbral dump. Trátalo como UN evento de mercado. Contar cinco emergencias es la trampa de principiante.",
        "lab.card.beginnerDump.title": "Drill principiante — flood dump",
        "lab.card.beginnerDump.body": "Practica: (1) pausa 60s (2) mira si BTC también está rojo (3) escribe una frase: ‘No hago nada / Tenía plan para X’. No revenge-trade.",
        "lab.card.idioDump.title": "Presión de un solo nombre",
        "lab.card.idioDump.body": "%@ en %@%% y el board no está todo rojo. Pregunta por QUÉ esta coin: unlock, news, flush de leverage — no ‘se murió el mercado’.",
        "lab.card.regimePump.title": "Pumps en cluster",
        "lab.card.regimePump.body": "%d pumps live. Drill: nombra tu FOMO en voz alta, luego revisa si tenías size/plan antes de las velas verdes.",
        "lab.card.singlePump.title": "Un solo pump",
        "lab.card.singlePump.body": "Una alerta verde ≠ tesis. Practica esperar una segunda confirmación (retest, volumen, BTC no dump) antes de pensar en actuar.",
        "lab.card.fees.title": "Fee desk caliente",
        "lab.card.fees.body": "Sats altos = mover on-chain sale caro. Aprende a separar ‘quiero mover coins’ de ‘el precio se mueve’. Las fees castigan la urgencia.",
        "lab.card.btcLead.title": "BTC lidera",
        "lab.card.btcLead.body": "BTC 24h %@%%. La mayoría de alts son beta de BTC. Si BTC dump, alts dump juntos es normal — no cinco crisis únicas.",
        "lab.card.near.title": "Cerca del trigger (heat)",
        "lab.card.near.body": "Heat del desk ~%d%%. Skill: nota cómo te sientes ANTES del fire. Pre-compromiso: ¿qué haría si cruza?",
        "lab.card.calm.title": "Tape calm — úsalo",
        "lab.card.calm.body": "Sin fires. Mejor momento para estudiar: journal de pings, afinar watchlist, ensayar reglas sin adrenalina.",
        "lab.card.quiet.title": "Quiet hours on",
        "lab.card.quiet.body": "Pings de mercado muteados. Lección ops: los pros agenda atención. Dormir > mirar un board rojo a las 3am.",
        "lab.card.vipNoise.title": "Sensibilidad VIP",
        "lab.card.vipNoise.body": "±2.5% dispara mucho. Aprende: sensibilidad ≠ edge. Si todo alerta, sube el umbral o acorta la watchlist.",
        "lab.card.history.title": "Revisa tu log de pings",
        "lab.card.history.body": "%d pings guardados. Drill: marca en cuáles habrías actuado vs ignorado. Patrón > predicción.",
        "lab.card.skill.title": "Skill del día",
        "lab.card.disclaimer.title": "No es consejo financiero",
        "lab.card.disclaimer.body": "LAB es overlay privado de entrenamiento para el admin. No recomienda trades, size ni timing. Tú respondes por cada decisión.",
        "lab.skill.r1": "Escribe un plan de 1 línea ANTES de cualquier alerta. Si no hay plan cuando dispara — default: solo observar.",
        "lab.skill.r2": "Separa urgencia P2P/ops del ruido de mercado. Una alerta dump no es un timer P2P.",
        "lab.skill.r3": "Practica size en papel: ‘Si me equivoco, ¿qué % me quita el sueño?’ Si no está claro, el size es grande.",
        "lab.skill.r4": "Aprende fricción de fees: cada move on-chain cuesta. La urgencia es cómo el principiante paga de más.",
        "lab.skill.r5": "Tras un flood dump, espera un cierre de vela (tu timeframe) antes de cambiar cualquier tesis.",
        "lab.concept.radar": "Concepto: radar ≠ señal de actuar",
        "lab.concept.correlation": "Concepto: correlación / régimen",
        "lab.concept.plan": "Concepto: plan antes del trigger",
        "lab.concept.idiosyncratic": "Concepto: riesgo idiosincrático",
        "lab.concept.fomo": "Concepto: control de FOMO",
        "lab.concept.confirmation": "Concepto: confirmación",
        "lab.concept.fees": "Concepto: conciencia de fees",
        "lab.concept.btcBeta": "Concepto: beta de BTC",
        "lab.concept.threshold": "Concepto: disciplina de umbral",
        "lab.concept.journal": "Concepto: journal en calma",
        "lab.concept.session": "Concepto: gestión de sesión",
        "lab.concept.sensitivity": "Concepto: sensibilidad de alertas",
        "lab.concept.review": "Concepto: review post-evento",
        "lab.concept.practice": "Concepto: práctica deliberada",

        "general.system": "Sistema",
        "general.launchAtLogin": "Abrir al iniciar sesión",
        "general.openLoginItems": "Abrir Ítems de inicio…",
        "general.loginBlocked": "macOS bloqueó el arranque. Aprueba Bonefeed en Ítems de inicio.",
        "general.runtime": "Funcionamiento",
        "general.pause": "Pausar actualizaciones",
        "general.sound": "Sonidos",
        "general.pollHint": "Actualiza cada ~10–15 s. Solo lectura — nunca opera.",

        "display.theme": "Tema",
        "display.themeHint": "El tema aplica al notch y al panel. Incluye Binance y Mac.",

        "language.title": "Idioma de la app",
        "language.hint": "Textos de Ajustes, guía, avisos y estado. Las etiquetas cyber (RADAR / SPOT) se mantienen.",
        "language.defaultNote": "Por defecto: inglés.",

        "guide.title": "Guía de uso",
        "guide.blurb": "Bonefeed mira saldos y te avisa. No compra, vende ni firma.",
        "guide.step1.title": "BINANCE",
        "guide.step1.body": "Ajustes → Binance → pega API solo lectura → Guardar → Probar.",
        "guide.step2.title": "NOTCH",
        "guide.step2.body": "Haz click en el island para PORT / EARN / SIG. Vive en el notch.",
        "guide.step3.title": "LOG",
        "guide.step3.body": "Si parpadea, abre LOG — ahí caen depósitos y señales.",
        "guide.step4.title": "SIGNALS",
        "guide.step4.body": "RADAR → SIGNALS es el desk de alerta temprana. VIP desbloquea el heat board tight.",
        "guide.openPanel": "Mostrar guía en el panel",
        "guide.footnote": "Solo lectura · sin trade · sin firma",

        "privacy.termsTitle": "Términos de uso",
        "privacy.privacyTitle": "Privacidad",
        "privacy.termsBody": """
Bonefeed es una herramienta informativa de solo lectura para uso personal.

• No ejecuta trades, transferencias, approvals ni firmas.
• Eres responsable de tus API keys, wallets que agregues y decisiones que tomes con los datos mostrados.
• Precios y saldos vienen de terceros (p. ej. Binance, mempool.space, RPCs públicos) y pueden ir retrasados o fallar.
• Nada en la app es consejo financiero, de inversión o fiscal.
• El software se ofrece “tal cual”, sin garantía. Úsalo bajo tu propio riesgo.
• No uses una API key de Binance con permisos de retiro o trading.
""",
        "privacy.privacyBody": """
Qué guardamos — solo en este Mac:

• API key/secret de Binance cifrados localmente (AES-GCM; clave en Keychain).
• Watchlist, wallets, umbrales, tema, idioma y posición del panel en UserDefaults / Application Support.
• Snapshots opcionales de portfolio para PnL 24h.

Qué no hacemos:

• Sin cuenta cloud de Bonefeed. Sin analytics. Sin venta de datos.
• Las credenciales no se suben a nuestros servidores (esta app no tiene backend propio).
• Las llamadas de red van a exchanges / APIs públicas que usas al activar la función (Binance, mempool, RPC).

Puedes borrar las credenciales en Ajustes → Binance → Borrar.
""",
        "privacy.ack": "Al usar Bonefeed aceptas estos términos y las notas de privacidad.",

        "radar.watchlist": "Lista de mercado",
        "radar.watchlistHint": "Precios y señales 24h en RADAR (ticker público Binance).",
        "radar.custom": "Otra (ej. ARB)",
        "radar.add": "Añadir",
        "radar.active": "Activas:",
        "radar.reset": "Reset a BTC · ETH · SOL",
        "radar.stablesNote": "Stables (USDT…) no sirven como ticker.",
        "radar.onchain": "On-chain",
        "radar.onchainHint": "Solo lectura BTC / ETH (+Aave HF) / SOL. Depósitos Binance cubren cualquier coin al acreditar.",

        "binance.apiTitle": "API (solo lectura)",
        "binance.apiHint": "API key de Binance solo lectura. Ve saldos — nunca retiros ni trades.",
        "binance.save": "Guardar",
        "binance.test": "Probar",
        "binance.clear": "Borrar",
        "binance.encrypted": "Cifrado en este Mac. Solo lectura — nunca compartas el secret.",
        "binance.streamLive": "User stream activo — depósitos más rápidos.",

        "wallets.watch": "Vigilar dirección (on-chain)",
        "wallets.hint": "Solo lectura. Saldo + avisos de depósito. Sin seed / sin gastar. MetaMask = pega tu address ETH.",
        "wallets.metamaskTip": "MetaMask → Detalles de cuenta → Copiar address. Pega el 0x… aquí. Solo lectura — Bonefeed no conecta ni firma.",
        "wallets.metamaskLabel": "Usar nombre “MetaMask”",
        "wallets.chain": "Red",
        "wallets.label": "Nombre",
        "wallets.add": "Añadir wallet",
        "wallets.empty": "Ninguna wallet vigilada.",
        "wallets.remove": "Quitar",
        "wallets.invalidBTC": "Usa address BTC (bc1… / 1… / 3…)",
        "wallets.invalidETH": "Usa address ETH / MetaMask 0x… (40 hex)",
        "wallets.invalidSOL": "Usa address SOL base58 (32–44 chars)",
        "wallets.invalidTitle": "Address inválida",

        "alerts.notifications": "Notificaciones macOS",
        "alerts.enable": "Activar notificaciones",
        "alerts.request": "Pedir permiso",
        "alerts.openSystem": "Ajustes del sistema…",
        "alerts.test": "Probar aviso",
        "alerts.signals": "Señales",
        "alerts.pumpDump": "Pump/dump 24h (watchlist)",
        "alerts.dump": "Dump ≤",
        "alerts.pump": "Pump ≥",
        "alerts.signalAssets": "Assets con señal (vacío = todas):",
        "alerts.filterOff": "Sin filtro · todas las del radar.",
        "alerts.fee": "Alerta fee BTC alta",
        "alerts.feeAt": "Fee ≥",
        "alerts.delivery": "Entrega",
        "alerts.cooldown": "Cooldown",
        "alerts.quiet": "Horas silenciosas",
        "alerts.from": "Desde",
        "alerts.to": "hasta",
        "alerts.quietHint": "Mute pump/dump/fee/earn/health. Depósitos + P2P sí.",
        "alerts.antiFlap": "Anti-flap (histéresis)",
        "alerts.antiFlapHint": "Dispara una vez al umbral; se re-arma solo cuando el precio recupera (~72% de la banda).",
        "alerts.volumeFilter": "Filtro de volumen (3×)",
        "alerts.volumeFilterHint": "Solo notifica pump/dump si el volumen 15m ≥ 3× el promedio previo.",
        "alerts.focus": "Respetar Focus / DND de macOS",
        "alerts.focusHint": "Mutea ruido de mercado con Focus on. Depósitos + P2P sí notifican.",
        "alerts.p2pRate": "Watch de tasa P2P",
        "alerts.p2pRateHint": "Requiere fiat concreto (no Auto). Avisa si el mejor anuncio cruza tu línea.",
        "alerts.p2pRateBuyMax": "Compra ≥ (fiat/u)",
        "alerts.p2pRateSellMin": "Venta ≤ (fiat/u)",
        "alerts.aave": "Aave (ETH)",
        "alerts.aaveHint": "Health Aave V3 en wallets ETH. Ignora si no usas Aave.",
        "alerts.health": "Alertas de health factor",
        "alerts.healthWarn": "Avisar si HF <",
        "alerts.p2p": "Binance P2P",
        "alerts.p2pHint": "Pro · solo lectura. Avisa cambios de estado de órdenes. API Reading.",
        "alerts.p2pEnable": "Alertas de órdenes P2P",
        "alerts.p2pFiat": "Filtro fiat",
        "alerts.p2pFiatAuto": "Auto (todas)",
        "msg.p2pNew": "P2P · NUEVA",
        "msg.p2pPaid": "P2P · PAGADA",
        "msg.p2pRelease": "P2P · LIBERANDO",
        "msg.p2pDone": "P2P · DONE",
        "msg.p2pCancel": "P2P · CANCEL",
        "msg.p2pAppeal": "P2P · APPEAL",
        "msg.p2pStatus": "P2P · UPDATE",
        "msg.p2pRateBuy": "P2P · TASA COMPRA",
        "msg.p2pRateBuyDetail": "%@/%@ mejor compra %.4f ≥ %.4f",
        "msg.p2pRateSell": "P2P · TASA VENTA",
        "msg.p2pRateSellDetail": "%@/%@ mejor venta %.4f ≤ %.4f",
        "menu.section.panel": "Panel",
        "menu.section.radar": "Radar",
        "menu.section.quick": "Atajos",
        "menu.showPanel": "Mostrar panel",
        "menu.hidePanel": "Ocultar panel",
        "menu.openOverview": "Abrir Overview",
        "menu.openP2P": "Abrir estado P2P",
        "menu.openSignals": "Abrir desk SIGNAL",
        "menu.openLab": "Abrir BOT (superadmin)",
        "menu.openBot": "Abrir BOT (superadmin)",
        "menu.settings": "Ajustes…",

        "about.mode": "Modo",
        "about.modeValue": "Market Radar + Binance + Earn",
        "about.chains": "Cadenas",
        "about.chainsValue": "BTC · ETH (+Aave HF) · SOL",
        "about.data": "Datos",
        "about.dataValue": "Binance · mempool · RPC · user stream",
        "about.secrets": "Secretos",
        "about.secretsValue": "AES-GCM + Keychain",
        "about.footer": "Ajustes aquí; el panel del island es lo operativo.",
        "about.showGuide": "Abrir guía de uso…",
        "about.madeBy": "Hecho por",
        "about.studio": "Vibes District",
        "about.site": "Sitio web",

        "panel.refreshHint": "Forzar refresh Spot / Earn / mercados",
        "panel.pnlBag": "cartera",
        "panel.pnlMkt": "mkt",
        "panel.tapLive": "tap LIVE para refrescar",
        "panel.apiOff": "API off · ⚙ Binance",
        "panel.marketsEmpty": "EMPTY · ⚙ Radar watchlist",
        "panel.logEmpty": "LOG EMPTY",
        "panel.logHint": "Depósitos, P2P y Earn aparecen aquí.",
        "panel.p2pOff": "Alertas P2P off · ⚙ Alerts",
        "panel.p2pEmpty": "SIN ÓRDENES P2P",
        "panel.p2pHint": "Solo lectura. Avisa NEW / PAID / DONE / CANCEL / APPEAL. Fiat en Ajustes → Alerts.",
        "panel.p2pDemoHint": "Prueba la card de orden en vivo sin una transferencia real. Las órdenes C2C reales siguen apareciendo si Binance está conectado.",
        "panel.simulateP2P": "> SIMULATE_P2P",
        "panel.advanceP2P": "> ADVANCE_PHASE",
        "panel.clearP2PDemo": "> CLEAR_DEMO",
        "p2p.card.waitBuyerPay": "Esperando pago del comprador…",
        "p2p.card.paySeller": "Paga al vendedor…",
        "p2p.card.releaseCrypto": "Pago recibido — libera el crypto…",
        "p2p.card.waitRelease": "Esperando que el vendedor libere…",
        "p2p.card.releasing": "Liberando crypto…",
        "p2p.card.done": "Trade completado",
        "p2p.card.cancelled": "Orden cancelada",
        "p2p.card.appeal": "Orden en apelación",
        "p2p.card.update": "Orden actualizada",
        "panel.simulate": "> SIMULATE_DEPOSIT",
        "panel.clearLog": "> CLEAR_LOG",
        "panel.noEarn": "NO EARN POSITIONS",
        "panel.noApi": "NO API  →  ⚙ SETTINGS",
        "panel.spotEmpty": "EMPTY  (funds may be in EARN)",
        "panel.pin": "Pin — deja el panel abierto",
        "panel.unpin": "Unpin",
        "notch.peekHint": "Click para abrir",
        "notch.p2pDockHint": "Abrir estado de orden P2P",
        "notch.alertHint": "Click para abrir LOG",
        "notch.alertSignalsHint": "Click para abrir desk SIGNAL",
        "notch.closePanel": "Cerrar panel",
        "notch.openPanel": "Abrir Bonefeed",

        "onboard.title": "BOOT // FIRST RUN",
        "onboard.settings": "> OPEN_SETTINGS",
        "onboard.toSignals": "> OPEN_SIGNAL_DESK",
        "onboard.ack": "> ACK",

        "msg.bootDetail": "Iniciando radar…",
        "msg.pasteKey": "Pega API key solo lectura y guarda.",
        "msg.keyRequired": "API key y secret son obligatorios.",
        "msg.savedEncrypted": "Guardado cifrado (sin prompts Keychain).",
        "msg.saveFailed": "En memoria esta sesión (falló guardar):",
        "msg.cleared": "Credenciales borradas.",
        "msg.saveFirst": "Guarda las keys primero.",
        "msg.apiMemory": "API en memoria.",
        "msg.apiReady": "API lista.",
        "msg.apiMigrated": "API migrada a almacén cifrado.",
        "msg.noApi": "Configura API en Settings",
        "msg.noApiDetail": "Sin API Binance — solo on-chain. Conecta keys para Spot.",
        "msg.testNotifyBody": "Si ves esto, las notificaciones macOS funcionan.",
        "msg.simDeposit": "Depósito simulado",
        "msg.simDetail": "Sim — así se ve un depósito entrando.",
        "msg.earnSubscribe": "Suscripción Earn",
        "msg.earnRedeem": "Redeem Earn",
        "msg.unlockSoon": "Unlock pronto",
        "msg.depositSimTitle": "Depósito simulado",

        "deposit.binanceOk": "Acreditado en Binance",
        "deposit.binancePending": "Depósito Binance",
        "deposit.onchainOk": "On-chain confirmado",
        "deposit.onchainMem": "On-chain mempool",
        "deposit.sim": "Depósito simulado",
        "signal.feesHigh": "Fees altas",
        "signal.btcFees": "BTC fees altas",
        "signal.dump": "dump 24h",
        "signal.pump": "pump 24h",
        "signal.signalDump": "Señal: %@ dump",
        "signal.signalPump": "Señal: %@ pump",
        "signal.regimeDump": "Dump de mercado",
        "signal.regimeDumpDetail": "%d / %d coins sobre umbral dump — un evento, no N emergencias.",
        "signal.calm": "Mercado calmado",
        "signal.healthLow": "Health baja",
        "signal.aaveRisk": "Aave health riesgo",
        "signal.radarOffline": "Radar offline",
        "status.healthLow": "Health factor Aave bajo — revisa colateral/deuda.",
        "status.pendingBtc": "BTC sin confirmar en mempool — Binance acredita al confirmar.",
        "status.zeroOnchain": "Radar activo. Balance on-chain 0 — aviso al primer depósito.",
        "status.listening": "Radar activo. Escuchando depósitos…",
        "status.earnSettling": "Account aún $%.2f con 0 posiciones (¿redeem pendiente?)",

        "notify.readyBody": "Radar listo para avisarte de depósitos y Earn.",
        "notify.notDetermined": "Sin permiso aún — pulsa Enable notifications",
        "notify.denied": "Denegadas — actívalas en Ajustes del Sistema → Notificaciones",
        "notify.authorized": "Activadas (revisa Ajustes del Sistema si no ves nada)",
        "notify.provisional": "Provisionales — busca Bonefeed en Notificaciones",
        "notify.ephemeral": "Efímeras",
        "login.enabled": "Activo — abrirá al iniciar sesión",
        "login.disabled": "Off",
        "login.pending": "Pendiente: aprueba Bonefeed en Login Items",
        "login.notFound": "Usa la app de ~/Applications y vuelve a activar",

        "err.missingCreds": "Faltan API key / secret",
        "err.badURL": "URL inválida",
        "err.decoding": "Respuesta Binance ilegible",
    ]
}
